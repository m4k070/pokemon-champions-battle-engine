...
describe('BattleEngine', () => {
  ...
  describe('Section: Attack Calculation', () => {
    test('should apply burn reduction to physical attacks', () => {
      ...
      const move = { name: 'flare-blitz', type: 'fire', power: 120, category: 'physical' };
      
      const attack = engine.calculateAttack(attacker, { category: 'physical' });
      expect(attack).toBeLessThan(120); // Burn reduces attack
    });
  });
  ...
  describe('Section: Defense Calculation', () => {
    test('should calculate defense correctly', () => {
      ...
      const defense = engine.calculateDefense(sylveon, { category: 'special' });
  ...
  describe('Section: Damage Calculation', () => {
    test('should calculate super effective damage', () => {
      ...
      const result = engine.useMove(attacker, defender, { 
        name: 'outrage', type: 'dragon', power: 120, accuracy: 100, category: 'physical', pp: 10 
      });
      
      expect(result.success).toBe(true);
      expect(result.effectiveness).toBe(2.0); // Dragon is 2x vs Fairy
    });
    ...
    test('should handle immunity', () => {
      ...
      const move = { name: 'earthquake', type: 'ground', power: 100, accuracy: 100, category: 'physical', pp: 10 };
      
      const result = engine.useMove(attacker, defender, { 
        name: 'earthquake', type: 'ground', power: 100, accuracy: 100, category: 'physical', pp: 10 
      });
      
      expect(result.effectiveness).toBe(0); // Ground has no effect on Flying
    });
  });
  ...
  describe('Abilities', () => {
    test('should activate intimidate on switch-in', () => {
      ...
      // Landorus Intimidate should reduce Garchomp's attack
  ...
  describe('Items', () => {
    test('should activate Focus Sash', () => {
      ...
      defender.takeDamage(100, { events: { emit: () => {} } });
  ...
  describe('Type Effectiveness', () => {
    ...
    test('should handle immunity', () => {
      ...
      const result = engine.calculateDamage(
        new Pokemon({name:'Garchomp', types:['dragon','ground'], ability:'rough-skin', item:null, baseStats:{HP:108,ATK:130,DEF:95,SPATK:80,SPDEF:85,SPEED:102}}),
        new Pokemon({name:'Togekiss', types:['fairy','flying'], ability:'serene-grace', item:'leftovers', baseStats:{HP:85,ATK:50,DEF:95,SPATK:120,SPDEF:115,SPEED:80}}),
  ...
  describe('Speed Calculation', () => {
    test('should apply Choice Scarf boost', () => {
      ...
      expect(engine.calculateSpeed(pokemon)).toBe(153); // 102 * 1.5
    });
