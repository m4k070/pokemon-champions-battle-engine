...
function buildContext(engine, side, self, selfTeam, opponent, opponentTeam) {
    return {
        turn: engine.turn,
        self,
        selfTeam,
...
export class BattleSession {
    ...
    reasoningLog = [];
    turnBegun = false;
    constructor(engine, teamA, teamB, activeA, activeB) {
        this.engine = engine;
        this.teamA = teamA;
        this.teamB = teamB;
    ...
    static async start(teamA, teamB, options = {}) {
    ...
    winner() {
        if (isTeamWiped(this.teamA))
            return 1;
        if (isTeamWiped(this.teamB))
            return 0;
        return null;
    }
    needsForcedSwitch(side) {
    ...
    applyForcedSwitch(side, decision) {
    ...
    applyTurn(decisionA, decisionB) {
        ...
        const attackingSides = [];
        if (actionA.type === 'move')
            attackingSides.push(0);
        if (actionB.type === 'move')
            attackingSides.push(1);
        attackingSides.sort((a, b) => this.engine.calculateSpeed(b === 0 ? this.activeA : this.activeB)
...
export async function runBattle(teamA, teamB, agentA, agentB, options = {}) {
...
//# sourceMappingURL=battle-runner.js.map