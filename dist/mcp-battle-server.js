...
export function createBattleServer() {
    ...
    server.registerTool('start_battle', {
    ...
    }, ({ teamA, teamB, leadIndexA, leadIndexB }) => handle(async () => {
        ...
        const stored = { history: new BattleHistory(session) };
        sessions.set(sessionId, stored);
        return { sessionId, state: stateView(stored) };
    }));
    server.registerTool('get_battle_state', {
        description: '指定したセッションの現在の盤面状態（両陣営のHP・技PP・天候・ステルスロック等）を取得する。',
        inputSchema: { sessionId: z.string() },
    }, ({ sessionId }) => handle(() => stateView(requireSession(sessionId))));
    server.registerTool('apply_forced_switch', {
        description: '瀕死になったポケモンの交代先を指定する。needsForcedSwitchSide0/1がtrueの側でのみ有効。'
    ...
    }, ({ sessionId, side, pokemonIndex, reasoning }) => handle(() => {
    ...
    server.registerTool('apply_turn', {
        description: '両陣営の行動を同時に適用し、1ターン進める。行動には具体的な行動'
    ...
    }, ({ sessionId, actionA, actionB, reasoningA, reasoningB }) => handle(async () => {
        const stored = requireSession(sessionId);
        const { session } = stored.history;
        stored.history.checkpoint();
        session.beginTurn();
    ...
    server.registerTool('fork_battle', {
        description: '現在の局面から独立した新しいセッションを複製する（別々の行動を試して比較する分岐探索用）。'
    ...
    server.registerTool('list_battles', {
        description: '現在サーバー上に存在するセッションIDの一覧を取得する。',
        inputSchema: {},
    }, () => handle(() => ({ sessionIds: [...sessions.keys()] })));
    return server;
}
//# sourceMappingURL=mcp-battle-server.js.map